...
def grouped_line_chart(
    ...
    x_lab_angle: float = -90,
    hline: Union[float, List[float]] = None,
    vline: Union[float, List[float]] = None,
    y_ci_lower_col: Optional[str] = None,
    y_ci_upper_col: Optional[str] = None,
    ordinal_date: bool = False,
    legend_off: bool = False,
    date_disp: str = "yearmonth",
    config: bool = True,
    ...
    Args:
        data (pd.DataFrame): dataframe containing data to be plotted
        x_col (str): column for x-axis
        y_col (str): column for y-axis
        color_col (str): column around which to base color
        stroke_col (str): column around which to base stroke type
        x_is_date (bool): if true, values in x-col are treated as temporal
        y_scale_fct (float): scaling factor for height of plot
        x_scale_fct (float): scaling factor for width of plot
        title (str): title for plot
        x_title (str): title for x-axis
        y_title (str): title for y-axis
        x_ticks (bool): if true, includes tick marks on the x-axis
        y_ticks (bool): if true, includes tick marks on the y-axis
        y_domain (tuple): parameters for lower and upper bounds of y-axis
        x_domain (tuple): parameters for lower and upper bounds of x-axis
        x_lab_angle (float): orientation of x label values from the x-axis
        hline (float, list): draws horizontal lines at the point(s) specified
        vline (float, list): draws vertical lines at the point(s) specified
        y_ci_lower_col (str): column for lower bound on error bars
        y_ci_upper_col (str): column for upper bound on error bars
        ordinal_date (bool): if true, dates are treated with ordinality
        legend_off (bool): if true, a legend will not be included
        date_disp (str): determines format for date display
        config (bool): if true, configuration parameters are included
        tooltip (list): columns to be included in tooltip
        highlight (bool): if true, allows for individual lines on plot to be highlighted
        point (bool): include dot at each observation

    Returns:
        (altair.Chart) line chart
    """
    data = data.copy()
    x_axis = alt.Axis(title=x_title, ticks=x_ticks, labelAngle=x_lab_angle)
    y_axis = alt.Axis(title=y_title, ticks=y_ticks)

    if y_domain:
        y_scale_kwargs = {"domain": [y_domain[0], y_domain[1]]}
    else:
        y_scale_kwargs = {}
    if x_domain:
        x_scale_kwargs = {"domain": [x_domain[0], x_domain[1]]}
    else:
        x_scale_kwargs = {}
    if tooltip is None:
        tooltip = []
    ...
    if not x_is_date:
        x_base = alt.X(f"{x_col}:Q", axis=x_axis, scale=alt.Scale(**x_scale_kwargs))
    else:
        x_type = "O" if ordinal_date else "T"
        x_base = alt.X(
            f"{date_disp}({x_col}):{x_type}",
            axis=x_axis,
            scale=alt.Scale(**x_scale_kwargs),
        )
    y_base = alt.Y(
        f"{y_col}:Q", scale=alt.Scale(zero=False, **y_scale_kwargs), axis=y_axis
    )

    legend = None if legend_off else alt.Legend()

    base = alt.Chart(data).mark_line(point=point).encode(x=x_base, y=y_base,
                                                     tooltip=tooltip)

    if color_col:
        color_base = alt.Color(f"{color_col}:N", legend=legend)
        base = base.encode(color=color_base)

    if stroke_col:
        stroke_base = alt.Stroke(f"{stroke_col}:N", legend=legend)
        base = base.encode(strokeDash=stroke_base)

    if y_ci_upper_col is not None or y_ci_lower_col is not None:
        if y_ci_upper_col is None or y_ci_lower_col is None:
            raise ValueError("Must supply upper and lower bounds for error band.")
        base = base + alt.Chart(data).mark_area(opacity=0.25).encode(
            x=x_base,
            y=y_ci_lower_col,
            y2=y_ci_upper_col,
        )
    ...
    if hline is not None:
        if type(hline) == list:
            ...
            y_points = [hline]
        rule = (
            alt.Chart(pd.DataFrame({"y": y_points}))
    ...
    if vline:
        if type(vline) == list:
            ...
            x_points = [vline]
        rule = (
            alt.Chart(pd.DataFrame({"x": x_points}))
            .mark_rule(strokeDash=[10, 10])
            .encode(x="x")
        )
        base = base + rule
    ...
    if highlight:
        selection_color = alt.selection_multi(fields=[f"{color_col}"], bind="legend")
        selection_stroke = alt.selection_multi(fields=[f"{stroke_col}"], bind="legend")

        base = (
            base.encode(
                opacity=alt.condition(
                    selection_color | selection_stroke, alt.value(1), alt.value(0.1)
                )
            )
            .add_selection(selection_stroke)
            .add_selection(selection_color)
        )

    if config:
        return base.properties(
            title=title, width=400 * x_scale_fct, height=400 * y_scale_fct
